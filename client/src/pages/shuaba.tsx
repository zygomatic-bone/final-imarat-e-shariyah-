...
import { useBranches, useJudges, type Branch, type Judge } from "@/lib/api";
...
export default function Shuaba() {
  ...
  const judges: Judge[] = judgesData?.items ?? [];
    ...
    <div className="min-h-screen" style={{ background: "var(--page-bg)" }}>
      ...
      <section className="pt-36 pb-20 px-6 text-center" style={{ background: "var(--page-bg)" }}>
        <Reveal>
          ...
          <h1 className="text-5xl sm:text-6xl font-black tracking-tight mb-3" style={{ color: "var(--text-primary)" }}>Shuabaat</h1>
          <p className="text-2xl mb-4" style={{ color: "#D4AF37", fontFamily: "'Noto Nastaliq Urdu', serif" }} dir="rtl">شعبہ جات</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {departments.map((d, i) => (
              <Reveal key={d.title} delay={i * 0.07}>
                <motion.div className={isDark ? "glass-card-dark" : "glass-card"}
                  ...
                  <p className="text-sm mb-1" dir="rtl" style={{ color: "#D4AF37", fontFamily: "'Noto Nastaliq Urdu', serif" }}>{d.titleUrdu}</p>
                  ...
                  <motion.button className="mt-5 flex items-center gap-1 text-xs font-semibold"
                    style={{ color: "#D4AF37" }} whileHover={{ x: 3 }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {judges.map((j, i) => (
              <Reveal key={j.id} delay={i * 0.08}>
                <motion.div className="glass-card-dark p-6 text-center"
                  style={{ border: "1px solid rgba(212,175,55,0.12)" }}
                  whileHover={{ scale: 1.02, boxShadow: "0 24px 60px rgba(0,0,0,0.6)" }}
                  transition={{ type: "spring", stiffness: 350, damping: 28 }}>
                  {j.photoUrl ? (
                    <img src={j.photoUrl} alt={j.name} className="w-14 h-14 rounded-2xl object-cover mx-auto mb-5" style={{ border: "1px solid rgba(212,175,55,0.18)" }} />
                  ) : (
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-5 text-xl"
                      style={{ background: "rgba(212,175,55,0.10)", border: "1px solid rgba(212,175,55,0.18)" }}>ع</div>
                  )}
                  ...
                  <p className="text-[11px]" dir="rtl" style={{ color: "rgba(212,175,55,0.60)", fontFamily: "'Noto Nastaliq Urdu', serif" }}>{j.nameUrdu}</p>
                </motion.div>
              </Reveal>
            ))}
          </div>
      ...
      <section id="branches" className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <Reveal delay={0.05}>
            <div className="flex items-center gap-3 px-4 py-3 rounded-2xl mb-8 max-w-sm"
              style={{
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
                placeholder="Search branches..." className="flex-1 bg-transparent text-sm outline-none"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredBranches.map((b, i) => (
              <Reveal key={b.id} delay={i * 0.06}>
                <motion.div className={isDark ? "glass-card-dark" : "glass-card"} style={{ padding: "28px" }}
                  whileHover={{ scale: 1.015, boxShadow: isDark ? "0 20px 50px rgba(0,0,0,0.5)" : "0 16px 48px rgba(0,0,0,0.09)" }}
                  transition={{ type: "spring", stiffness: 400, damping: 30 }}>
                  ...
                  <p className="text-[13px] mb-4 leading-relaxed" style={{ color: "var(--text-secondary)" }}>{b.description}</p>
      ...
      {/* Terms */}
      <section className="py-20 px-6" style={{ background: isDark ? "#080808" : "#F4F4F4" }}>
              ...
              <Reveal key={i} delay={i * 0.05}>
                <div className="flex items-start gap-4 p-5 rounded-2xl" style={{
                  ...
                  <span className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 mt-0.5"
                    style={{ background: "rgba(212,175,55,0.12)", color: "#B8960C" }}>{i + 1}</span>
